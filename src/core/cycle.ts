...
async function runPhaseEmbed(engine: BrainEngine, dryRun: boolean, sourceId?: string, signal?: AbortSignal): Promise<PhaseResult> {
  try {
    ...
    // Fork: also thread sourceId so a default-source cycle scopes embed to the
    // resolved source instead of globally sweeping unrelated stale sources.
    const result = await runEmbedCore(engine, { stale: true, dryRun, sourceId, signal });
  ...
  try {
    ...
    if (phases.includes('embed')) {
      ...
      if (!engine) {
        ...
        const { result, duration_ms } = await timePhase(() => runPhaseEmbed(engine, dryRun, cycleSourceId, opts.signal));
        result.duration_ms = duration_ms;
        phaseResults.push(result);
        progress.finish();
      }
